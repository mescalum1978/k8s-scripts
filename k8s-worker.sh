...
echo "[1/6] Updating system, disable swap..."
sudo apt update && sudo apt upgrade -y

sudo swapoff -a
sudo sed -i '/ swap / s/^/#/' /etc/fstab

echo "[2/6] Install prerequisite packages, htop is optional..."
sudo apt install -y apt-transport-https ca-certificates curl gpg htop

echo "[3/6] Install and configure containerd..."
...
echo "[4/6] Add Kubernetes $K8S_VERSION repo and install..."
...
echo "[5/6] Apply Sysctl settings..."
